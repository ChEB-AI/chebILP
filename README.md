...
This writes to `data/chebi_v251/`:
...
Steps 2 and 3 write files into `data/ilp_problems/` (one subdirectory per class).
The default predicate set is `atoms`. Most other predicate sets extend `atoms`. 
Some predicate sets are experimental. We recommend  `--predicate_set chembl_fgs` 
which provides additional predicates based on functional groups.

**Optional: LLM-generated auxiliary predicates (experimental)**

The `llm_generated_rules` predicate set augments `atoms` with
*auxiliary predicates* invented by an LLM. 
Each auxiliary predicate is defined by an ASP rule.
The predicate set offered to the LLM is `atoms` with an extension for molecular weight
and ring sizes.
...
You can choose a model with `--model provider/name` (via [LiteLLM](https://github.com/BerriAI/litellm)); it defaults to
`anthropic/claude-haiku-4-5`. Also tested: `anthropic/claude-opus-4-8` and `openai/gpt-5.6-sol`.
 The model must support structured outputs. The
provider's API key is read from `.env` / the environment under its standard name
(`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, ...). 

Then run `build_bk` with `--predicate_set llm_generated_rules --predicate_dir <library>`;
...
There is also a version of this workflow that produces _Python programs_ instead of ASP rules:
```bash
python -m chebILP.predicate_generation.generate_auxiliary_predicates \
  --labels_file data/chebi_v251/ChEBI25_3_STAR/labels.txt \
  --chebi_version 251 \
  --n_predicates 4 \
  --predicate_dir data/llm_generated_python
```
---
