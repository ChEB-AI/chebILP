...
class ChEBIDataset:
    ...
    def __init__(
        ...
        self.molecules_path = os.path.join(self.processed_dir, "molecules.pkl")
    ...
    def _download_if_needed(self) -> None:
        if not os.path.exists(self.obo_path):
            ...
            download_chebi_obo(self.chebi_version, dest_dir=self.raw_dir)
        if not os.path.exists(self.sdf_path):
            print(f"Downloading chebi.sdf.gz v{self.chebi_version}...")
            download_chebi_sdf(self.chebi_version, dest_dir=self.raw_dir)
    ...
    def select_labels(
        ...
        labeled_df, labels = build_labeled_dataset(
            ...
            min_molecules=self.min_pos_samples,
        )
