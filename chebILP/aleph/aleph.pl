...
update_best(S,Clause,PCover,NCover,Label/_,Label1/Node1,Label1/Node1):-
	...
	retractall('$aleph_search'(selected,_)),
        ...
        % chebILP: stamp the elapsed time-to-best each time a new global best is accepted,
        % in Aleph's own stopwatch clock (see induce_start in induce/0). run_aleph.pl prints it.
        ( stopwatch(Now), '$aleph_global'(induce_start,T0) ->
              TB is Now - T0,
              retractall('$aleph_global'(time_to_best,_)),
              asserta('$aleph_global'(time_to_best,TB))
        ; true ),
        arg(35,S,VSearch),
        (VSearch = true ->
...
update_best(S,Clause,_,_,Label/Node,Label1/Node1,Label/Node):-
        ...
        (VSearch = true ->
                ...
                true),
	update_good(Label1,Clause),
        % "good" clauses (better than parent, not the global best) are printed once per
        % node and dominate the search trace; gate them behind verbosity so verbosity 0
        % keeps only the far rarer "newbest" clauses. See show_clause(newbest,...) above.
        ((setting(verbosity,V), V >= 1) -> show_clause(good,Label1,Clause,Node1) ; true),
        record_clause(good,Label1,Clause,Node1).
...
induce:-
	...
	record_settings,
        stopwatch(StartClock),
        % chebILP: publish the induce start clock so update_best/7 can record the
        % elapsed time-to-best in the same (stopwatch/cputime) clock as [search time].
        retractall('$aleph_global'(induce_start,_)),
        retractall('$aleph_global'(time_to_best,_)),
        asserta('$aleph_global'(induce_start,StartClock)),
        repeat,
        gen_sample(pos,S),
